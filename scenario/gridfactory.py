...
def create_net() -> dict[str, Any]:
    ...
    pp.create_lines(
        ...
        names=["line_center_battery", "line_battery_ev", "line_battery_pv", "line_center_grid"],
    )
    
    pp.create_ext_grid(net, bus=bgrid, vm_pu=1.0, name="Grid_Connection")
    
    '''
    pp.create_gen(net, bus=bpv, p_mw=0.0, name="PV_Generator")
    pp.create_load(net, bus=bev, p_mw=0.0, q_mvar=0.0, name="EV_Load")
    
    pp.create_gen(net, bus=bbat, p_mw=0.0, name="Battery_Inverter")
    pp.create_load(net, bus=bbat, p_mw=0.0, q_mvar=0.0, name="Battery_Load")
    '''
    
    return {"net": net, "id_center": bcenter, "id_battery": bbat, "id_ev": bev, "id_pv": bpv, "id_grid": bgrid}
